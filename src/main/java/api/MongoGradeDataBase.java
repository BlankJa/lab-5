...
public class MongoGradeDataBase implements GradeDataBase {
    ...
    public Team getMyTeam() {
        ...
        try {
            final Response responseExec = client.newCall(request).execute();
            final JSONObject body = new JSONObject(responseExec.body().string());
            if (body.getInt(STATUS_CODE) == SUCCESS_CODE) {
